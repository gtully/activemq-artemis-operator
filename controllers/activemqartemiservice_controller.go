...
func (reconciler *ActiveMQArtemisServiceInstanceReconciler) processBroker() error {
	...
	var index int32
	for index = 0; index < getSpecSize(reconciler.instance); index++ {
		...
		desired.Spec.DeploymentPlan.Labels = map[string]string{
			fmt.Sprintf("%s-peer-index", reconciler.instance.Name): fmt.Sprintf("%v", index),
		...
		// if index > 1 - we need to deal with some choeragraphy with their respective properties secret extra mounts
...
func (reconciler *ActiveMQArtemisServiceInstanceReconciler) processStatus(cr *broker.ActiveMQArtemisService, reconcilerError error) (err error) {
	...
	var ready int32
	var index int32
	for index = 0; index < getSpecSize(cr); index++ {
		...
		if obj != nil {
			...
			if brokerReady != nil && brokerReady.Status == metav1.ConditionTrue {
				...
				deployedCondition.Message = fmt.Sprintf("ordinal[%d] not ready status %v", index, deployed.Status)
			}
			break
		}
	}
	if ready == getSpecSize(cr) {
...
func getSpecSize(cr *broker.ActiveMQArtemisService) int32 {
	if cr.Spec.Size != nil {
		return *cr.Spec.Size
	}
	return 1
