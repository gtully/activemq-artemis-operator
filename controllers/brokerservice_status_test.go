	...
	Context("status condition transitions", func() {
		...
		It("should track Deployed and Ready conditions correctly", func() {
			...
			Eventually(func(g Gomega) {
				...
				deployedCond := meta.FindStatusCondition(createdCrd.Status.Conditions, brokerv1beta1.DeployedConditionType)
				if deployedCond != nil {
					if verbose {
			...
			app := brokerv1beta1.BrokerApp{
				...
				Spec: brokerv1beta1.BrokerAppSpec{
					...
					Capabilities: []brokerv1beta1.AppCapabilityType{
						{
							Role:       "workQueue",
							ProducerOf: []brokerv1beta1.AppAddressType{{Address: "STATUS.QUEUE"}},
						},
					},
				},
			}
		...
		It("should handle app capability updates", func() {
			...
			app := brokerv1beta1.BrokerApp{
				...
				Spec: brokerv1beta1.BrokerAppSpec{
					...
					Capabilities: []brokerv1beta1.AppCapabilityType{
						{
							Role:       "workQueue",
							ProducerOf: []brokerv1beta1.AppAddressType{{Address: "INITIAL.QUEUE"}},
							ConsumerOf: []brokerv1beta1.AppAddressType{{Address: "INITIAL.QUEUE"}},
						},
					},
				},
			}
			...
			Eventually(func(g Gomega) {
				...
				createdApp.Spec.Capabilities = append(createdApp.Spec.Capabilities, brokerv1beta1.AppCapabilityType{
					...
					ConsumerOf: []brokerv1beta1.AppAddressType{
						{Address: "UPDATED.QUEUE"},
					},
				})
