...
var _ = Describe("artemis-service", func() {
	...
	Context("round trip simple", func() {
		...
		It("non persistent", func() {
			...
			app := brokerv1beta1.ActiveMQArtemisApp{
				...
				Spec: brokerv1beta1.ActiveMQArtemisAppSpec{
					...
					Capabilities: []brokerv1beta1.AppCapabilityType{
						{
							//Role: single identity role for MTLS
							//ProducerOf: []brokerv1beta1.AppAddressType{{QueueName: "APP.JOBS"}},
							//ConsumerOf: []brokerv1beta1.AppAddressType{{QueueName: "APP.JOBS"}},
			...
			buf := &bytes.Buffer{}
			fmt.Fprintf(buf, "amqps://%s:%d", serviceName, ServicePort)
